...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Assuming your backend registration endpoint is 'auth/register'
      await request("auth/register", "POST", formData);
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        // Fallback if request utility throws the error data directly
        const backendErrors: Record<string, string> = {};
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className={`h-10 w-full rounded border px-3 text-sm outline-none transition focus:ring-2 ${errors.name
